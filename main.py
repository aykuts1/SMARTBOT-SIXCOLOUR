...
Stake: her işlem açılışında ANLIK olarak Bybit'ten bakiye okunup %2 alınır.
       Periyodik stake güncellemesi yoktur.
"""
import logging
import signal
import sys
...
class Bot:
    ...
    def __init__(self):
        ...
        # KIRMIZI grubu thread'leri (blue → yellow → red sırasıyla, bağlar için)
        self.blue_thread = BlueThread(self.cfg, self.dm, self.tm)
        self.yellow_thread = YellowThread(self.cfg, self.dm, self.tm, self.blue_thread)
        self.red_thread = RedThread(self.cfg, self.dm, self.tm,
                                    self.blue_thread, self.yellow_thread)

        # BEYAZ grubu thread'leri (purple → orange → white)
        self.purple_thread = PurpleThread(self.cfg, self.dm, self.tm)
        self.orange_thread = OrangeThread(self.cfg, self.dm, self.tm, self.purple_thread)
    ...
    def start_trading(self):
        ...
        if not self.red_thread.is_alive():
            self.red_thread = RedThread(self.cfg, self.dm, self.tm,
                                        self.blue_thread, self.yellow_thread)
        self.yellow_thread.set_blue_ref(self.blue_thread)
        ...
        if not self.white_thread.is_alive():
            self.white_thread = WhiteThread(self.cfg, self.dm, self.tm,
                                            self.purple_thread, self.orange_thread)
        self.orange_thread.set_purple_ref(self.purple_thread)
